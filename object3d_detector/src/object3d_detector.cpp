...
typedef struct feature {
  ...
  float slice[20];
  float intensity[27];
  /*** for storing human point cloud***/
  pcl::PointCloud<pcl::PointXYZI> human_pc;
...
public:
  ...
  void setROI(pcl::PointCloud<pcl::PointXYZI>::Ptr initial_pc,pcl::PointCloud<pcl::PointXYZI>::Ptr filter_pc);

};

void Object3dDetector::setROI(pcl::PointCloud<pcl::PointXYZI>::Ptr initial_pc,pcl::PointCloud<pcl::PointXYZI>::Ptr filter_pc)
{
  // Define min and max for X, Y and Z
  float minX = -10.0, minY = -10.0, minZ = -0.8;
  float maxX = +10.0, maxY = +10.0, maxZ = +1.2;

  pcl::CropBox<pcl::PointXYZI> boxFilter;
  boxFilter.setMin(Eigen::Vector4f(minX, minY, minZ, 1.0));
  boxFilter.setMax(Eigen::Vector4f(maxX, maxY, maxZ, 1.0));
  boxFilter.setInputCloud(initial_pc);
  boxFilter.filter(*filter_pc);
}
...
Object3dDetector::Object3dDetector() {
  ...
  // point_cloud_sub_ = node_handle_.subscribe<sensor_msgs::PointCloud2>("/velodyne_clouded", 1, &Object3dDetector::pointCloudCallback, this);
  ...
  human_pose_pub = private_nh.advertise<geometry_msgs::Pose>("human_pose",100);
...
void Object3dDetector::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2) {
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr result_pc(new pcl::PointCloud<pcl::PointXYZI>);

  setROI(pcl_pc,result_pc);
  
  // extractCluster(pcl_pc);
  extractCluster(result_pc);
  classify();
...
void Object3dDetector::extractFeature(pcl::PointCloud<pcl::PointXYZI>::Ptr pc, Feature &f,
  ...
  (f.human_pc).resize (pc->size());
  for (std::size_t i = 0; i < (f.human_pc).size (); ++i)
  {
      (f.human_pc)[i].x = (*pc)[i].x;
      (f.human_pc)[i].y = (*pc)[i].y;
      (f.human_pc)[i].z = (*pc)[i].z;
      (f.human_pc)[i].intensity = (*pc)[i].intensity;
  } 
...
void Object3dDetector::classify() {
  ...
  for(std::vector<Feature>::iterator it = features_.begin(); it != features_.end(); ++it) {
    ...
    //  std::cout << ", number_points = " << it->number_points << ", min_distance = " << std::sqrt(it->min_distance) << std::endl;
	...
	if(prob_estimates[0] < human_probability_)
      ...
      std::string default_s = "human_box_";
      std::string end = ".pcd";
      std::string s = std::to_string(count);
      std::string filename = default_s + s + end;
      count++;

      if(count <= 1000)
      {
        // pcl::io::savePCDFile(filename, (it->human_pc));//Save the point cloud to a PCD file
