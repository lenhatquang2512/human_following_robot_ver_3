...
{
    ros::init(argc,argv,"icart_pid_each_velo_loop");
    ros::NodeHandle nh;
    ...
    float target_goal_linear =  1.0f;

    float step_angular = deg2rad(90.0);
    float step_linear  = 0.5f;
    ...
    while(true)
    {
        ...
        while (ros::ok())
        {
            ...
            stateDiffs_for_angle_z = targetGoal_angle_z - icartState.getYaw();
            ...
            if(std::abs(stateDiffs_for_angle_z) <= tolerance)
            {
                ...
                // target_goal_angular += icartState.getYaw() ;
                target_goal_angular = icartState.getYaw() +  step_angular;
                break;
            }
        ...
        while (ros::ok())
        {
            ...
            if(std::abs(stateDiffs_for_x) <= tolerance)
            {
                ...
                target_goal_linear = distance + step_linear ;
            ...
            printf("Num = %d\n", i);
            printf("x = %f, y = %f, distance = %f \n", icartState.getX(), icartState.getY(),distance);
            printf("YAW (degree) = %f \n", rad2deg(icartState.getYaw()));
            printf("vx = %f\n", icartState.getVx());
        ...
        std::string lower_str = boost::algorithm::to_lower_copy(user_command);
        lower_str.compare("yes");

        if(lower_str.compare("yes") == 0)
        {

            std::cout << "GOAL ANGULAR IS  " << rad2deg(target_goal_angular) << "[deg] " << std::endl;
            std::cout << "GOAL LINEAR IS " << target_goal_linear << "[m]" << std::endl;
            // icartState.setX(0.0);
            // icartState.setYaw(0.0);

        }
        else if (lower_str.compare("no") == 0)
        {
        ...
        {
            std::cout <<"DONT UNDERSTAND, TYPE AGAIN" << std::endl;
            break;
        }
